...
use super::math::set_theory;

const U64MIN: u64 = u64::MIN;  // 0x0000000000000000;
const U64MAX: u64 = u64::MAX;  // 0xffffffffffffffff;
const S64MIN: i64 = i64::MIN;  // -0x8000000000000000;
const S64MAX: i64 = i64::MAX;  // 0x7fffffffffffffff;
...
impl StridedInterval {
    fn new(stride: u64, min: i64, max: i64, offset: i64) -> StridedInterval {
        StridedInterval {
            stride: stride,
            min: min,
            max: max,
    ...
    fn from_const(con: i64) -> StridedInterval {
        StridedInterval {
            stride: 1,
            min: con,
            max: con,
...
impl fmt::Display for StridedInterval {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match (self.stride, self.offset) {
            ...
            (1, _)
                => write!(f, "[{}, {}] + {}", self.min, self.max, self.offset),
            (_, 0)
                => write!(f, "{}[{}, {}]", self.stride, self.min, self.max),
            (_, _)
                => write!(f, "{}[{}, {}] + {}", self.stride, self.min, self.max, self.offset),
        }
    }
}
